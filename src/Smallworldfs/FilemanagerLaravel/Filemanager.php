        ...
        if ($this->is_image($imagePath) && $this->config['images']['resize']['enabled']) {
            if ($size = @getimagesize($imagePath)) {
                if ($size[0] > $this->config['images']['resize']['maxWidth'] || $size[1] > $this->config['images']['resize']['maxHeight']) {
                    ...
                    $resized = $image->resize($this->config['images']['resize']['maxWidth'], $this->config['images']['resize']['maxHeight'], function ($constraint) {
            ...
            if (isset($_GET['mode']) && $_GET['mode'] != '') {
                switch ($_GET['mode']) {
                    ...
                    case 'getinfo':
                        if ($this->getvar('path')) {
                            $current_path = $this->getFullPath();
                            if (!$this->isValidPath($current_path)) $this->error("No way.");
